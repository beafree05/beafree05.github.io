...
const VOCAB_PROMPT_TEMPLATE = `\u4f60\u662f\u4e00\u4e2a\u4e13\u4e1a\u7684\u65e5\u8bed\u6559\u5e08\uff0c\u76ee\u6807\u662f\u5e2e\u52a9\u5b66\u4e60\u8005\u6df1\u5165\u7406\u89e3\u65e5\u8bed\u8bcd\u6c47\u3002

\u5f53\u7528\u6237\u8f93\u5165\u4e00\u4e2a\u65e5\u8bed\u5355\u8bcd\u65f6\uff0c\u8bf7\u6309\u4ee5\u4e0b\u7ed3\u6784\u8fdb\u884c\u8be6\u7ec6\u8bb2\u89e3\uff1a
...
\u6ce8\u610f\uff1a
- \u4e0d\u8981\u7f16\u9020\u5177\u4f53\u8f9e\u4e66\u51fa\u5904
- \u5982\u679c\u65e0\u6cd5\u786e\u8ba4\u8bfb\u97f3\u6216\u7528\u6cd5\uff0c\u8bf7\u660e\u786e\u5199\u300c\u9700\u8981\u786e\u8ba4\u300d
- \u4f8b\u53e5\u5fc5\u987b\u81ea\u7136\uff0c\u7b26\u5408\u65e5\u672c\u4eba\u5b9e\u9645\u8868\u8fbe\u4e60\u60ef
- \u8f93\u51fa\u8bed\u8a00\u4ee5\u4e2d\u6587\u8bf4\u660e\u4e3a\u4e3b\uff0c\u65e5\u8bed\u4f8b\u53e5\u4fdd\u7559\u65e5\u8bed`;

let wanakanaBoundInput = null;
...
function initVocabJudge() {
  ...
  analyzeWordBtn.addEventListener("click", async () => {
    ...
    jpWordInput.value = normalizeLookupInput(jpWordInput.value);
...
function buildVocabularyReport({ word, contextNote }) {
  const rawWord = normalizeLookupInput(word);
...
async function buildVocabularyReportAsync({ word, contextNote }) {
  ...
  return buildVocabularyReport({ word: rawWord, contextNote: note });
}
...
function renderVocabularyReport(report) {
  const teacherReport = normalizeTeacherReport(report);
  readingChip.textContent = `\u5047\u540d\uff1a${teacherReport.reading}`;
  usageChip.textContent = `\u5e38\u7528\u5ea6\uff1a${teacherReport.frequency}`;

  const basicInfoMarkup = teacherReport.basicInfo
    .map(
      (item) => `
  ...
  const chineseMeaningMarkup = teacherReport.meanings
    .map(
      (item) => `
        <div class="analysis-list-item">
  ...
  const usageMarkup = teacherReport.usageItems
    .map(
      (item) => `
        <div class="analysis-list-item">
          <strong>${escapeHtml(item.label)}</strong>
  ...
  const exampleMarkup = teacherReport.examples
    .map(
      (item, index) => `
        <div class="example-item">
          <strong>${escapeHtml(`\u4f8b\u53e5 ${index + 1}`)}</strong>
          <div class="analysis-text">${escapeHtml(item.ja)}</div>
          <span>\u4e2d\u6587\uff1a${escapeHtml(item.zh)}</span>
          <span>\u8bf4\u660e\uff1a${escapeHtml(item.note)}</span>
        </div>
      `
    )
    .join("");

  const nuanceMarkup = teacherReport.nuanceItems
    .map(
      (item) => `
        <div class="analysis-list-item">
          <strong>${escapeHtml(item.label)}</strong>
  ...
  vocabResult.innerHTML = `
    <div class="analysis-block">
      ...
      <div class="analysis-list">${basicInfoMarkup}</div>
    </div>
    <div class="analysis-block">
      <div class="analysis-title">\u30102. \u4e2d\u6587\u542b\u4e49\u3011</div>
      <div class="analysis-list">${chineseMeaningMarkup}</div>
    </div>
    <div class="analysis-block">
      <div class="analysis-title">\u30103. \u7528\u6cd5\u8bf4\u660e\u3011</div>
      <div class="analysis-list">${usageMarkup}</div>
    </div>
    <div class="analysis-block">
      <div class="analysis-title">\u30104. \u4f8b\u53e5\u3011</div>
      <div class="example-list">${exampleMarkup}</div>
    </div>
    <div class="analysis-block">
      <div class="analysis-title">\u30105. \u8bed\u611f / \u6ce8\u610f\u70b9\u3011</div>
      <div class="analysis-list">${nuanceMarkup}</div>
    </div>
    <div class="analysis-block">
      <div class="analysis-title">\u30106. \u5b66\u4e60\u63d0\u793a\u3011</div>
      <div class="analysis-text">${escapeHtml(teacherReport.learningTip)}</div>
    </div>
    <div class="analysis-block">
      <div class="analysis-title">\u6559\u5b66\u5907\u6ce8</div>
      <div class="analysis-text">${escapeHtml(teacherReport.teacherNote)}</div>
    </div>
    <div class="analysis-block">
...
function normalizeTeacherReport(report) {
  const writing = report.writing || report.input || "-";
  const reading = report.reading || "\u8981\u78ba\u8a8d";
  const romanized = buildRomaji(reading);
  const meanings = buildMeaningItems(report);
  const examples = buildTeacherExamples(report);
  const usageSummary = buildUsageSummary(report.usage);
  const collocations = buildCollocationText(report);
  const nuanceItems = buildNuanceItems(report);

  return {
    writing,
    reading,
    frequency: report.frequency || "\u8981\u78ba\u8a8d",
    basicInfo: [
      { label: "\u5355\u8bcd", value: writing },
      { label: "\u5047\u540d", value: reading },
      { label: "\u7f57\u9a6c\u97f3", value: romanized || "\u53ef\u9009 / \u6682\u4e0d\u5c55\u793a" },
      { label: "\u8bcd\u6027", value: report.partOfSpeech || "\u8981\u78ba\u8a8d" }
    ],
    meanings,
    usageItems: [
      { label: "\u662f\u5426\u5e38\u7528", value: `${report.frequency || "\u8981\u78ba\u8a8d"} / ${buildCommonWordHint(report.frequency)}` },
      { label: "\u4f7f\u7528\u573a\u666f", value: usageSummary },
      { label: "\u642d\u914d\u6216\u56fa\u5b9a\u8868\u8fbe", value: collocations }
    ],
    examples,
    nuanceItems,
    learningTip: buildLearningTip(report, reading, romanized),
    teacherNote: report.sourceSummary || "\u5f53\u524d\u7ed3\u679c\u4ee5\u672c\u5730\u6559\u5b66\u6574\u7406\u4e3a\u4e3b\u3002"
  };
}

function buildMeaningItems(report) {
  const result = [];
  const splitCn = String(report.cnExplanation || "")
    .split(/(?<=[。；])/)
    .map((item) => item.trim())
    .filter(Boolean);

  if (splitCn.length) {
    splitCn.slice(0, 3).forEach((item, index) => {
      result.push({
        title: index === 0 ? "\u6700\u5e38\u89c1\u542b\u4e49" : `\u542b\u4e49 ${index + 1}`,
        body: item
      });
    });
  }

  if (!result.length && Array.isArray(report.dictExplanation) && report.dictExplanation.length) {
    report.dictExplanation.slice(0, 3).forEach((item, index) => {
      result.push({
        title: index === 0 ? "\u6700\u5e38\u89c1\u542b\u4e49" : `\u542b\u4e49 ${index + 1}`,
        body: item
      });
  ...
  if (!result.length) {
    result.push({
      title: "\u6700\u5e38\u89c1\u542b\u4e49",
      body: "\u5f53\u524d\u8fd8\u9700\u8981\u7ed3\u5408\u771f\u5b9e\u8bed\u5883\u8fdb\u4e00\u6b65\u786e\u8ba4\u3002"
    });
  }

  if (Array.isArray(report.glossDefinitionsEn) && report.glossDefinitionsEn.length) {
    result.push({
      title: "\u8f85\u52a9\u7406\u89e3",
      body: `English gloss: ${report.glossDefinitionsEn.join(" / ")}`
    });
  }
...
function buildTeacherExamples(report) {
  const examples = Array.isArray(report.examples) ? report.examples.filter((item) => item && item.ja && item.ja !== "-") : [];
  if (examples.length) {
    return examples.slice(0, 3).map((item) => ({
      ja: item.ja,
      zh: item.zh || "-",
      note: item.why || item.alternative || "\u8fd9\u4e2a\u4f8b\u53e5\u4e3b\u8981\u5e2e\u4f60\u89c2\u5bdf\u8be5\u8bcd\u5728\u771f\u5b9e\u8bed\u5883\u91cc\u7684\u7528\u6cd5\u3002"
    }));
  }
  ...
  return [
    {
      ja: "\u6682\u65f6\u8fd8\u6ca1\u6709\u751f\u6210\u53ef\u4f9b\u53c2\u8003\u7684\u81ea\u7136\u4f8b\u53e5\u3002",
      zh: "\u6682\u65f6\u8fd8\u6ca1\u6709\u53ef\u5c55\u793a\u7684\u4f8b\u53e5\u3002",
      note: "\u5efa\u8bae\u540e\u7eed\u7ed3\u5408\u771f\u5b9e\u6765\u6e90\u8865\u8db3 2~3 \u4e2a\u4f8b\u53e5\u518d\u4e3b\u52a8\u4f7f\u7528\u3002"
    }
  ];
}
...
function buildCollocationText(report) {
  const examples = Array.isArray(report.examples) ? report.examples.filter((item) => item && item.ja && item.ja !== "-") : [];
  const hints = [];

  if (report.partOfSpeech && report.partOfSpeech.includes("\u52d5\u8a5e")) {
    hints.push("\u5148\u89c2\u5bdf\u5b83\u5e38\u63a5\u4ec0\u4e48\u4e3b\u8bed\u3001\u5bbe\u8bed\u6216\u52a9\u8bcd");
  } else if (report.partOfSpeech && report.partOfSpeech.includes("\u540d\u8a5e")) {
    hints.push("\u6ce8\u610f\u5b83\u662f\u72ec\u7acb\u4f7f\u7528\uff0c\u8fd8\u662f\u66f4\u5e38\u51fa\u73b0\u5728\u56fa\u5b9a\u642d\u914d\u91cc");
  } else if (report.partOfSpeech && report.partOfSpeech.includes("\u5f62\u5bb9")) {
    hints.push("\u6ce8\u610f\u5b83\u540e\u9762\u5e38\u63a5\u7684\u540d\u8bcd\u7c7b\u578b\uff0c\u4ee5\u53ca\u662f\u5426\u5e26\u6709\u660e\u663e\u8bc4\u4ef7\u8272\u5f69");
  }

  if (examples.length) {
    hints.push(`\u53ef\u5148\u4ece\u4f8b\u53e5\u91cc\u8bb0\u4f4f\u8fd9\u7c7b\u7528\u6cd5\uff1a${examples[0].ja}`);
  }

  return hints.join("\uff1b") || "\u5f53\u524d\u5148\u901a\u8fc7\u4f8b\u53e5\u89c2\u5bdf\u5b83\u7684\u7fd2\u6163\u642d\u914d\u3002";
}

function buildNuanceItems(report) {
  const items = [];

  if (Array.isArray(report.synonyms) && report.synonyms.length) {
    items.push({
      label: "\u4e0e\u76f8\u4f3c\u8bcd\u7684\u533a\u522b",
      value: report.synonyms.map((item) => `${item.word}\uff1a${item.note}`).join(" / ")
    });
  }

  items.push({
    label: "\u5e38\u89c1\u9519\u8bef\u7528\u6cd5",
    value: report.notFitScenes || "\u6682\u65f6\u8fd8\u9700\u8981\u6839\u636e\u771f\u5b9e\u8bed\u5883\u8fdb\u4e00\u6b65\u786e\u8ba4\u3002"
  });

  items.push({
    label: "\u4f7f\u7528\u65f6\u9700\u8981\u6ce8\u610f",
    value: report.advice || "\u5efa\u8bae\u5148\u5f53\u6210\u7406\u89e3\u578b\u8bcd\u6c47\uff0c\u518d\u51b3\u5b9a\u662f\u5426\u4e3b\u52a8\u4f7f\u7528\u3002"
  });

  return items;
}

function buildCommonWordHint(frequency) {
  switch (frequency) {
    case "\u9ad8":
      return "\u504f\u5e38\u7528\uff0c\u503c\u5f97\u4f18\u5148\u5b66";
    case "\u4e2d":
      return "\u6709\u4e00\u5b9a\u5e38\u89c1\u5ea6\uff0c\u9002\u5408\u7406\u89e3\u548c\u79ef\u7d2f";
    case "\u4f4e":
      return "\u4e0d\u7b97\u9ad8\u9891\uff0c\u5148\u7406\u89e3\u5373\u53ef";
    default:
      return "\u5e38\u7528\u5ea6\u8fd8\u9700\u8981\u7ee7\u7eed\u786e\u8ba4";
...
function buildLearningTip(report, reading, romanized) {
  const writing = report.writing || report.input || "\u8fd9\u4e2a\u8bcd";
  const meaning = Array.isArray(report.dictExplanation) && report.dictExplanation.length
    ? report.dictExplanation[0]
    : report.cnExplanation || "\u5b83\u7684\u6838\u5fc3\u542b\u4e49";
  const readingHint = reading && reading !== "\u8981\u78ba\u8a8d"
    ? `\u5148\u628a\u300c${writing} = ${reading}${romanized ? ` (${romanized})` : ""}\u300d\u8fd9\u5bf9\u5e94\u5173\u7cfb\u8bb0\u4f4f`
    : `\u5148\u7ed3\u5408\u8bcd\u5178\u786e\u8ba4 ${writing} \u7684\u8bfb\u97f3`;

  return `${readingHint}\uff0c\u7136\u540e\u518d\u628a\u5b83\u7ed1\u5230\u4e00\u53e5\u6700\u81ea\u7136\u7684\u4f8b\u53e5\u4e0a\u53bb\u8bb0\u3002\u4e0e\u5176\u5b64\u7acb\u80cc\u5355\u8bcd\uff0c\u4e0d\u5982\u8bb0\u4f4f\u201c${meaning}\u201d\u8fd9\u4e2a\u6838\u5fc3\u611f\u89c9\u3002`;
}
...
function buildRomaji(reading) {
  ...
  const kataToHira = value.replace(/[\u30a1-\u30f6]/g, (char) =>
  ...
  let result = "";
  for (let index = 0; index < kataToHira.length; index += 1) {
    const current = kataToHira[index];
    const pair = kataToHira.slice(index, index + 2);
    if (digraphMap[pair]) {
    ...
    if (current === "\u3063") {
      const nextPair = kataToHira.slice(index + 1, index + 3);
      const nextRomaji = digraphMap[nextPair] || kanaMap[kataToHira[index + 1]] || "";
...
function setupWanakanaInput() {
  const wanakana = getWanakana();
  if (!wanakana || !jpWordInput || wanakanaBoundInput === jpWordInput) {
    return;
  }

  wanakana.bind(jpWordInput, {
    IMEMode: true
  });
  wanakanaBoundInput = jpWordInput;

  jpWordInput.addEventListener("blur", () => {
    jpWordInput.value = normalizeLookupInput(jpWordInput.value);
  });
}

function getWanakana() {
  return globalThis.wanakana || null;
}

function normalizeLookupInput(word) {
...
function createEmptyVocabularyReport() {
  return {
    ...
    suggestions: [],
    sourceType: "empty",
    sourceSummary: "\u8f93\u5165\u76ee\u6807\u8bcd\u540e\uff0c\u9875\u9762\u4f1a\u57fa\u4e8e\u5f53\u524d\u5185\u7f6e\u8bcd\u5e93\u4e0e\u6559\u5b66\u6a21\u677f\u751f\u6210\u8be6\u7ec6\u8bb2\u89e3\u3002"
  };
}
...
function createFallbackVocabularyReport(rawWord, note, normalized) {
  ...
  return {
    ...
    suggestions: [],
    sourceType: "fallback",
    sourceSummary: "\u5f53\u524d\u8bcd\u6761\u4e0d\u5728\u5185\u7f6e\u8bcd\u5e93\u91cc\uff0c\u56e0\u6b64\u9875\u9762\u7ed9\u51fa\u7684\u662f\u4fdd\u5b88\u7248\u5b66\u4e60\u5206\u6790\uff0c\u4e0d\u5efa\u8bae\u76f4\u63a5\u62ff\u53bb\u505a\u6b63\u5f0f\u8f93\u51fa\u4f9d\u636e\u3002"
  };
}
